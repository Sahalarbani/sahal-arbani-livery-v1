...
export default function Loading() {
  return (
    ...
    <div className="min-h-screen bg-brand-dark pt-28 px-4 flex flex-col items-center justify-start relative overflow-hidden">
      ...
      <div className="mb-12 relative z-10 flex flex-col items-center gap-6 mt-10">
        <div className="p-4 bg-white/5 backdrop-blur-xl border border-white/5 rounded-full shadow-lg animate-pulse">
            <Loader2 className="w-8 h-8 text-brand-accent animate-spin" />
        </div>
        <h2 className="text-brand-accent font-bold text-xs md:text-sm uppercase tracking-widest animate-pulse opacity-80">
            Establishing Link...
        </h2>
      </div>
      ...
      <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10 pb-20">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          ...
          <div key={i} className="bg-brand-onyx/40 backdrop-blur-xl border border-white/5 rounded-[32px] h-[380px] w-full relative overflow-hidden p-4 flex flex-col gap-5 shadow-sm">
            ...
            {/* Bottom Decoration */}
            <div className="mt-auto h-1 w-full bg-gradient-to-r from-transparent via-brand-accent/20 to-transparent rounded-full opacity-50"></div>
          </div>
        ))}
      </div>
    </div>
