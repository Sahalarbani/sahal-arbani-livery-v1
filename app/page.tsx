    ...
    <div className="relative min-h-screen overflow-hidden bg-transparent">
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl grid-cols-1 gap-10 px-4 pb-12 pt-32 sm:px-6 lg:grid-cols-[1.02fr_0.98fr] lg:px-8 lg:pb-16 lg:pt-36">
        <div className="flex flex-col justify-center">
          <div className="mb-7 inline-flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 backdrop-blur-xl">
          ...
          <h1 className="max-w-4xl text-5xl font-black leading-[0.95] tracking-tight text-white sm:text-6xl lg:text-7xl">
            SAHAL ARBANI
            <span className="mt-2 block bg-gradient-to-r from-brand-cyan via-brand-sage to-brand-accent bg-clip-text font-display text-transparent">
              LIVERY
            </span>
          </h1>
          ...
          <p className="mt-7 max-w-2xl text-base leading-8 text-slate-300 sm:text-lg">
            ...
            <Link href="https://www.tiktok.com/@sahal.arbani" target="_blank" className="ml-1 font-semibold text-brand-cyan transition-colors hover:text-white">@sahal.arbani</Link>
          </p>

          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <Link href="#gallery" className="inline-flex items-center justify-center gap-2 rounded-full bg-brand-cyan px-6 py-3 text-sm font-black uppercase tracking-[0.14em] text-brand-dark shadow-halo transition-all hover:bg-white active:scale-95">
        ...
        <div className="glass-panel relative overflow-hidden rounded-[28px] p-3">
          <div className="absolute inset-x-10 top-0 h-px bg-gradient-to-r from-transparent via-brand-cyan/70 to-transparent" />
          <div className="relative aspect-[4/3] overflow-hidden rounded-[22px] bg-brand-onyx">
            {skins[0]?.image ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-brand-dark via-brand-dark/12 to-transparent" />
            <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-4">
              ...
              <div className="hidden rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-right backdrop-blur-xl sm:block">
                <p className="text-2xl font-black text-white">{skins.length}</p>
                <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-slate-400">Assets</p>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2 pt-3">
            {skins.slice(0, 3).map((skin) => (
              <div key={skin.id} className="hairline-panel flex items-center gap-2 rounded-2xl p-2">
                <div className="relative h-10 w-12 shrink-0 overflow-hidden rounded-xl bg-brand-dark">
                  {skin.image && <img src={skin.image} alt="" className="h-full w-full object-cover" />}
                </div>
                <div className="min-w-0">
                  <p className="truncate text-[10px] font-bold uppercase tracking-wide text-white">{skin.category}</p>
                  <p className="flex items-center gap-1 text-[10px] text-slate-400"><Download size={10} /> {(skin.downloads || 0).toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <form action="/" method="GET" className="glass-panel relative flex items-center rounded-full p-2">
          ...
          <input
            ...
            className="w-full bg-transparent px-4 py-3 text-sm font-semibold tracking-wide text-white outline-none placeholder:text-slate-500"
          />
        </form>
        <AdSlot 
      ...
      <div id="gallery" className="relative z-10 mx-auto max-w-7xl px-4 pb-24 pt-14 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-black uppercase tracking-tight text-white sm:text-3xl">Latest Liveries</h2>
            <p className="mt-2 text-sm text-slate-400">Pilih kategori atau cari asset TOE3 favoritmu.</p>
          </div>
        ...
        <div className="mb-10 flex flex-wrap gap-2 rounded-[22px] border border-white/10 bg-white/[0.045] p-2 backdrop-blur-xl">
            ...
            return (
              <Link
                ...
                className={`
                  ...
                  ${isActive 
                    ? 'bg-white text-brand-dark shadow-halo' 
        ...
        {skins.length === 0 ? (
          <div className="glass-panel flex flex-col items-center justify-center rounded-[28px] border-dashed py-24">
            ...
            <p className="text-lg font-semibold uppercase tracking-wide text-slate-300">No Data Found</p>
            <p className="mt-1 text-sm text-slate-500">Try adjusting your search parameters.</p>
          </div>
        ) : (
