...
import { ArrowLeft, Edit3, Zap, Globe, Lock, FileText, Loader2 } from "lucide-react";
  ...
  return (
    <button type="submit" disabled={pending} className={`w-full relative bg-brand-accent hover:bg-brand-accent/90 text-brand-dark font-black text-sm py-4 md:py-5 rounded-full shadow-lg transition-all flex items-center justify-center gap-3 group overflow-hidden ${pending ? 'opacity-70 cursor-wait' : ''}`}>
      ...
      <span className="flex items-center gap-3 relative z-10">
        {pending ? <><Loader2 size={20} className="animate-spin" /> PUBLISHING...</> : <><Zap size={20} strokeWidth={2.5} /> PUBLISH ARTICLE</>}
      </span>
    </button>
  );
}
...
export default function CreateBlogPage() {
  ...
  const [formData, setFormData] = useState({
    title: "", excerpt: "", content: "", tag: "Tutorial", readTime: "5 min read", published: true,
  });

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const { name, value, type } = e.target;
    if (name === "tagSelect") {
      if (value === "custom") { setIsCustomTag(true); setFormData(p => ({ ...p, tag: "" })); }
      else { setIsCustomTag(false); setFormData(p => ({ ...p, tag: value })); }
      return;
    }
    ...
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="bg-brand-onyx border border-white/5 rounded-3xl md:rounded-[40px] p-4 md:p-10 shadow-sm relative overflow-hidden">
        <div className="flex items-center gap-4 mb-8 md:mb-10 pl-2">
          ...
          <div>
            <h1 className="text-xl md:text-3xl font-black text-gray-100 uppercase tracking-tight">New Article</h1>
            <p className="text-gray-500 text-[10px] font-semibold uppercase tracking-widest mt-1">Compose & Publish</p>
          </div>
        </div>
        ...
        <form action={dispatch} className="space-y-6 md:space-y-8">
          <div className="space-y-5 md:space-y-6 p-4 md:p-6 bg-brand-dark/50 rounded-2xl md:rounded-3xl border border-white/5">
            <div>
              ...
              <input name="title" type="text" value={formData.title} onChange={handleInputChange} placeholder="e.g. Cara Pasang Livery di TOE3" className={`w-full bg-brand-dark border ${state.errors?.title ? 'border-red-500' : 'border-white/5'} rounded-full p-4 text-gray-100 font-bold focus:border-brand-accent focus:outline-none transition-colors text-sm`} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                ...
                <select name="tagSelect" onChange={handleInputChange} className="w-full bg-brand-dark border border-white/5 rounded-full p-4 text-gray-200 font-semibold text-xs focus:border-brand-accent focus:outline-none cursor-pointer appearance-none">
                  {TAG_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}
                  <option value="custom">+ Custom</option>
                </select>
                {isCustomTag && <input type="text" name="tag" value={formData.tag} onChange={handleInputChange} placeholder="TYPE TAG..." className="w-full bg-brand-accent/10 border border-brand-accent/50 rounded-full p-4 text-brand-accent font-bold uppercase tracking-widest focus:outline-none text-xs mt-2" />}
                {!isCustomTag && <input type="hidden" name="tag" value={formData.tag} />}
              </div>
              <div>
          ...
          <div className="space-y-5 md:space-y-6 p-4 md:p-6 bg-brand-dark/50 rounded-2xl md:rounded-3xl border border-white/5">
            <div>
              <label className="block text-[10px] font-bold text-gray-400 border-l border-brand-accent pl-2 uppercase tracking-widest mb-2 ml-2">Excerpt (Preview Text)</label>
              <textarea name="excerpt" rows={2} value={formData.excerpt} onChange={handleInputChange} placeholder="Ringkasan singkat yang muncul di blog index..." className={`w-full bg-brand-dark border ${state.errors?.excerpt ? 'border-red-500' : 'border-white/5'} rounded-2xl p-4 text-gray-300 focus:border-brand-accent focus:outline-none text-sm leading-relaxed`} />
            </div>
            ...
            <div>
              ...
              <textarea name="content" rows={12} value={formData.content} onChange={handleInputChange} placeholder="<h2>Judul Section</h2><p>Isi artikel di sini...</p>" className={`w-full bg-brand-dark border ${state.errors?.content ? 'border-red-500' : 'border-white/5'} rounded-2xl p-4 text-gray-300 focus:border-brand-accent focus:outline-none font-mono text-xs leading-relaxed`} />
